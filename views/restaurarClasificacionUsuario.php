...
<nav class="navbar navbar-default" role="navigation">
      ...
      <div class="col-md-2" align="center">
	   ...
	   ?>
       		<div class="well well-small alert alert-block" align="center">
    			...
    			<h4>No existen registros en clasificación de usuario para restaurar</h4>
   			</div>
  ...
  <tbody>
      <?php
	  if(count($rowClasifUsuario->return)>1){   
		for ($i=0;$i<count($rowClasifUsuario->return);$i++){
			echo '<tr>';
			echo '<td style="text-align:center" width="10%">'.$rowClasifUsuario->return[$i]->id.'</td>';
			echo '<td width="20%">'.$rowClasifUsuario->return[$i]->nombre.'</td>';
			echo '<td width="25%">'.$rowClasifUsuario->return[$i]->descripcion.'</td>';
			echo '<td style="text-align:center" width="15%"><input type="checkbox" name="ide['.$i.']" id="ide['.$i.']" value='.$rowClasifUsuario->return[$i]->id.'></td>';
			echo '</tr>';
			}
	  }
	  else{
		  	echo '<tr>';
			echo '<td style="text-align:center" width="10%">'.$rowClasifUsuario->return->id.'</td>';
			echo '<td width="20%">'.$rowClasifUsuario->return->nombre.'</td>';
			echo '<td width="25%">'.$rowClasifUsuario->return->descripcion.'</td>';
			echo '<td style="text-align:center" width="15%"><input type="checkbox" name="ide[0]" id="ide[0]" value='.$rowClasifUsuario->return->id.'></td>';
			echo '</tr>';
